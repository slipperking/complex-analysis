...
def coords_to_typst_array(coords: np.ndarray) -> str:
    ...
    items = ", ".join(f"({x:.6f}, {y:.6f})" for x, y in coords)
    return f"({items})"
...
def read_input_json(filename: Path):
    ...
    subcover_disk_radius_scalar = float(
        data.get("subcover_disk_radius_scalar", 1.0))
...
def tikz_bezier_sample(points, tension: float = 0.55, samples_per_seg: int = 20):
    ...
    for i in range(n):
        ...
        for t in np.linspace(0, 1, samples_per_seg, endpoint=False):
            B = (1 - t) ** 3 * p1 + 3 * (1 - t) ** 2 * t * \
                c1 + 3 * (1 - t) * t ** 2 * c2 + t ** 3 * p2
            out.append(B)
    return np.array(out)
...
def write_region_boundaries(folder: str, geom_list, prefix: str, list_macro: str,
    ...
    rings = []
    polys = [
        g for geom in geom_list
        for g in (geom.geoms if isinstance(geom, (MultiPolygon, GeometryCollection)) else [geom])
    ]
    for poly in polys:
    ...
    with open(all_path, "w") as f:
        if generate_individuals:
            ...
            f.write("\n".join(import_lines) + "\n")
            list_value = "(" + ", ".join(ref_names) + ")"
            f.write(typst_let(list_var, list_value))
            print(f"  Written list variable {list_var} with {len(ref_names)} references to {all_path}")
            ...
            if len(path_values) == 1:
                list_value = f"({path_values[0]},)"
            else:
                inner = ",\n  ".join(path_values)
                list_value = f"(\n  {inner},\n)"

            f.write(typst_let(list_var, list_value))
            print(f"  Written list variable {list_var} with {len(path_values)} inline paths to {all_path}")
...
def write_input_disks(folder: str, disk_points: List[np.ndarray], disk_r: float):
    ...
    all_path = os.path.join(folder, "all.typ")
    with open(all_path, "w") as f:
        import_lines = "\n".join(f'#import "{i+1}.typ": {ref_names[i]}' for i in range(len(ref_names)))
        f.write(import_lines + "\n")
        list_value = "(" + ", ".join(ref_names) + ")"
        f.write(typst_let("input-disk-list", list_value))
    ...
    with open(centers_path, "w") as f:
        f.write(typst_let("input-disk-centers", f"({items})"))
...
# ---------------------------------------------------------------------------
# Subcover helpers (unchanged geometry logic)
# ---------------------------------------------------------------------------

def prune_redundant_disks(centers: List[np.ndarray], r_sub: float,
                           covered_region, nocenters_region) -> List[np.ndarray]:
    if not centers:
        return []
    ...
    sorted_idx = sorted(range(len(centers)),
                        key=lambda i: useful_area_values[i], reverse=True)
...
def main():
    ...
    K = compute_input_region(
        cfg["pre_erosion_curves"], cfg["samples_per_segment"])
    Eroded = compute_eroded_region(
        cfg["pre_erosion_curves"], cfg["global_offset"], cfg["samples_per_segment"])
    Support = compute_support_region(
        cfg["support_curves"], cfg["samples_per_segment"])
    ...
    D = unary_union([Point(p).buffer(disk_r, resolution=50)
                     for p in cfg["disk_points"]]) if cfg["disk_points"] else None
    NoCentersRegion = unary_union([g for g in [K, D] if g])

    for name, geom in [
        ("K_region", K), ("ErodedRegion", Eroded), ("support_region", Support),
    ...
    final_union = unary_union([Point(p).buffer(r_sub, resolution=80)
                               for p in sub_centers]) if sub_centers else Polygon()
