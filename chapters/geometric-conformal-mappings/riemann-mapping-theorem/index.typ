...
#proof[
  ...
  Additionally, since $forall k in NN$, $abs(tilde(alpha)_(n_k)) < 1$ in $U$, it follows that $f(U) subset.eq overline(DD)$. By the Open Mapping Theorem (@thm:open-mapping), the condition becomes $f(U) subset.eq DD$. Since $tilde(alpha)_(n_k) (z_0) = 0$ for all $k in NN$ and $tilde(alpha)_(n_k) (z_0) -> 0 = f(z_0)$, it follows that $f in cal(F)$.
  ...
  Fix $xi in psi(U')$ to be arbitrary. By the Open Mapping Theorem (@thm:open-mapping), there exists an open neighborhood $D(xi, epsilon) subset.eq psi(U')$. It follows that $D(-xi, epsilon) inter psi(U') = emptyset$. Therefore, $forall z in U'$, $abs(psi(z) + xi) >= epsilon$, and consequently, $abs(1 / (psi(z) + xi)) <= 1 / epsilon$. Hence, the function $phi(z) = 1 / (z + xi)$ maps $U'$ to a bounded region that lies within the compact disk $overline(D(0, 1 / epsilon))$. Denote $phi compose psi(U')$ by $tilde(U)$.
  ...
  $
  by Cauchy--Goursat (@thm:cauchy-goursat-theorem), since $U$ is simply connected by assumption, $rho^(-1) compose psi^(-1) compose phi^(-1)(Gamma)$ is a closed piecewise $C^1$ curve in $U$, and the integrand is holomorphic on $U$. Therefore, $tilde(U)$ is simply connected.
