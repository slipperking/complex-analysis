...
#theorem[Solution to the Dirichlet Problem][
  Let $U$ be an open region such that for each point $p in partial U$, there is a barrier $b_p : overline(U) -> RR$. Then $forall f in C(partial U, RR)$, $exists tilde(f) in C(overline(U), RR)$ harmonic in $U$ such that $tilde(f) equiv f$ on $partial U$.
]
#proof[
