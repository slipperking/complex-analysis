...
#proposition[
  ...
  + For any multi-index $bold(alpha) = (alpha_1, dots, alpha_n)$ with $abs(bold(alpha)) = r <= d$,
    $
  ...
  + If $r = d != 0$, then $partial^(bold(alpha)) bold(psi)$ is constant (and there exists a multi-index $bold(alpha)$ with $abs(bold(alpha)) = d$ such that $partial^(bold(alpha)) bold(psi)$ is nonzero). <itm:homogeneouspolynomialderivatives_equality>
...
#proof[
  ...
  $
    (k_1 - alpha_1) + dots + (k_n - alpha_n) = d - abs(bold(alpha)).
  $
  Hence, $partial^(bold(alpha)) bold(psi)$ is a homogeneous polynomial of degree $d - abs(bold(alpha))$, establishing @itm:homogeneouspolynomialderivatives_less.
...
#lemma("Cartan")[
  ...
  $
    bold(J)_(bold(f))(bold(a)) = mat(
      pdv(f_1, z_1)(bold(a)), dots, pdv(f_1, z_n)(bold(a));
      dots.v, dots.down, dots.v;
      pdv(f_n, z_1)(bold(a)), dots, pdv(f_n, z_n)(bold(a))
    ) = bold(I) = mat(
      1, dots, 0;
      dots.v, dots.down, dots.v;
      0, dots, 1
...
#proof[
  ...
  Since $bold(psi)_m equiv.not 0$, there exists $bold(alpha)$ satisfying $abs(bold(alpha)) = m$ such that
  $
...
#proof[
  ...
  By @lem:multivarcartan2, the map $bold(phi) compose bold(f)$ is linear, so $bold(phi) compose bold(f)(bold(z)) = bold(A) bold(z)$ for some invertible constant matrix $bold(A) = mat(zeta_(1,1), dots, zeta_(1,n); dots.v, dots.down, dots.v; zeta_(n,1), dots, zeta_(n,n))$. Thus,
  $
  ...
  $
    (bold(phi) compose bold(f))(bold(z)_(j,k)) = (1 - 1 / k)(sum_(i=1)^n (abs(zeta_(j,i)) / zeta_(j,i)) zeta_(1,i), dots, sum_(i=1)^n (abs(zeta_(j,i)) / zeta_(j,i)) zeta_(j,i), dots, sum_(i=1)^n (abs(zeta_(j,i)) / zeta_(j,i)) zeta_(n,i)) in DD^n.
  $
  ...
  As $k -> oo$,
  $
    sum_(i=1)^n abs(zeta_(j,i)) in overline(DD) quad forall j in {1, dots, n}.
  ...
  $
  As $k -> oo$, $bold(z)_(j,k) -> bold(e)_j in partial(DD^n)$ (the $j$-th unit basis vector), so the limit
  $
    (zeta_(1,j), dots, zeta_(n,j)) in partial(DD^n)
  $
  holds by continuity (extending across the boundary in this direction). Thus, $max_(i in {1, dots, n}) abs(zeta_(i,j)) = 1$. Combined with @eq:holomorphicautomorphismgrouponpolydisk_absolutesumestimate, this forces exactly one entry in the $j$-th column of $bold(A)$ to have absolute value $1$ (of the form $ee^(ii theta_j)$), with all others zero.
...
#proof[
  ...
  $
    norm(bold(f)(bold(z)))^2 = norm(bold(U) bold(z))^2 = bold(z)^dagger bold(U)^dagger bold(U) bold(z) = bold(z)^dagger bold(z) = norm(z)^2,
  $
...
#proof[
  ...
  $
    bold(phi)_(abs(bold(alpha))) compose bold(U) bold(f) equiv bold(V) ==> bold(f)(bold(z)) equiv bold(U)^(-1) bold(phi)_(abs(bold(alpha)))^(-1) compose bold(V) bold(z).
  $
