...
proved that the unit ball $B^n={(z_1,dots,z_n) in CC^n : sum_(k=1)^n abs(z_k)^2<1}$ and the polydisk $DD^n=DD times dots.c times DD$ are not biholomorphically equivalent by comparison of their automorphism groups, under certain assumptions of the automorphisms on their respective boundaries; the proof was later formalized by Cartan, but is nonetheless largely attributed to Poincaré. As one of the first of many deviations, the miracle of the Riemann Mapping Theorem (@thm:riemann-mapping) fails in higher dimensions. Hartogs later also showed that poles and essential singularities cannot exist as isolated singularities of multivariate holomorphic functions. Perhaps these are the results of an unsatisfactory generalization.
