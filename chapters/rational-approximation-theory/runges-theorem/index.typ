...
#proof[
  ...
  $
  The distance between $K$ and $partial tilde(K)$ is strictly positive. Suppose instead that the distance were zero. Then some point of $K$ would lie on the boundary of a square $Q in cal(G)$ and also lie on $partial tilde(K)$. If this point lies on an edge of $Q$ (but not at a vertex), then the square adjacent along that edge must also intersect $K$, and hence belong to $cal(G)$, contradicting the assumption that the point lies on $partial tilde(K)$. If the point lies at a vertex of $Q$, then all three adjacent squares also intersect $K$, so they too belong to $cal(G)$, leading to the same contradiction. Thus, the distance must be positive.
