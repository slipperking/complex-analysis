...
$ cal(F)\cbraces{K_lambda}(x)=cal(F)^{-1}\cbraces{K_lambda}(x)=k_lambda(x), cal(F)\cbraces{k_lambda}(x)=cal(F)^{-1}\cbraces{k_lambda}(x)=K_lambda(x), $
 where we define 
$ cal(F)\cbraces{f}(x)=\frac1{sqrt(2 pi)}integral_{-oo}^oo f(t)ee^{-ii xt}ddt, cal(F)^{-1}\cbraces{f}(x)=\frac1{sqrt(2 pi)}integral_{-oo}^oo f(t)ee^{ii xt}ddt. $] <lem:wienerikehara_kernels_fouriertransform>
...
$ upright("I")=integral_0^{plus.minus oo}(sin u)/(2u)dif u,\qfor plus.minus oo= cases(+oo & x+2 lambda>0, ,
 -oo & x+2 lambda<0,) $
 and substituting $u=t(x-2 lambda)$ gives 
$ \mathrm{I\!I}=integral_0^{plus.minus oo}(sin u)/(2u)dif u,\qfor plus.minus oo= cases(+oo & x-2 lambda>0, ,
 -oo & x-2 lambda<0.) $
...
#theorem("name=_Prime Number Theorem_,store=thm:primenumber")[The prime counting function $pi(x)$ is asymptotically equal to $\frac x{log x}$, i.e., $\lim_{x ->+oo}\frac{pi(x)log x}x=1$.] <thm:primenumber>
