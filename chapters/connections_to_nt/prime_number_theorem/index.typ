...
$ psi(x)=\mathmakebox[\widthof{\(sum\)}][c]{sum_{\substack{p lt.eq x\\p " prime"}}}sum_{k:p^k lt.eq x}log p=\mathmakebox[\widthof{\(sum\)}][c]{sum_{\substack{p lt.eq x\\p " prime"}}}sum_{k lt.eq log_p x}log p=\mathmakebox[\widthof{\(sum\)}][c]{sum_{\substack{p lt.eq x\\p " prime"}}}sum_{k=1}^{\floor{(log x)/(log p)}}log p=sum_{\substack{p:p lt.eq x\\p " prime"}}\floor{(log x)/(log p)}log p. $ <eq:chebyshevfunction_psi_floor>

Hence we have for any $x>1$,

$ 0 lt.eq theta.alt(x)lt.eq psi(x)lt.eq \mathmakebox[\widthof{\(sum_{p:p lt.eq x}\)}][c]{sum_{\substack{p:p lt.eq x\\p " prime"}}}(log x)/(log p)log p=\mathmakebox[\widthof{\(sum\)}][c]{sum_{\substack{p:p lt.eq x\\p " prime"}}}log x=pi(x)log x. $ <eq:chebyshevfunctions_bounds_primecounting>
...
$ f(s)=-\frac1s (zeta'(s))/(zeta(s))=> f(s)-(1)/(s-1)=-\frac1s \qty((zeta'(s))/(zeta(s))+\frac1{s-1})-\frac1s. $ <eq:primenumbertheorem_laplacetransformchebyshevfunction_gfunction>
