...
$ Gamma(z)=\lim_{n -> oo}(n^z n!)/(product_(k=0)^n(z+k)), Re z>0. $ <eq:gammafunctiongaussformula>] <thm:gammafunctiongaussformula>
...
$ \lim_{n -> oo}[integral_0^n ee^{-t}t^{z-1}ddt-f_n(z)]=\lim_{n -> oo}integral_0^n ee^{-t}t^{z-1}[1-ee^{t}{(1-(t)/(n))}^n]ddt. $ <eq:gammafunction_gaussformulaintermediate1>
