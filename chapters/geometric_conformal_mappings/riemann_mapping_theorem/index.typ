...
#proof[
  ...
  Fix $xi in psi(U')$ to be arbitrary. By the Open Mapping Theorem (@thm:openmapping), there exists an open neighborhood $D(xi, epsilon) subset.eq psi(U')$. It follows that $D(-xi, epsilon) inter psi(U') = emptyset$. Therefore, $forall z in U'$, $abs(psi(z) + xi) >= epsilon$, and consequently, $abs(1 / (psi(z) + xi)) <= 1 / epsilon$. Hence, the function $phi(z) = 1 / (z + xi)$ maps $U'$ to a bounded region that lies within the compact disk $overline(D(0, 1 / epsilon))$. Denote $phi compose psi(U')$ by $tilde(U)$.
