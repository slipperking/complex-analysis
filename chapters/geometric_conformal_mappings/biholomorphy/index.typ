...
#proof[
  ...
  By @thm:hurwitzshifts, then there exists $delta > 0$ such that $forall epsilon in D(0, delta)$, $f(z) - w_0 - epsilon$ has exactly one zero in $overline(D(z_0, rho))$. In other words, $forall w_0 in f(U)$, $exists delta > 0$ such that $forall w in D(w_0, delta)$, $exists! z in overline(D(z_0, rho))$ such that $f(z) = w$. Thus, $D(w_0, delta) subset.eq f(U)$. Thus, $f(U)$ is an open set since each contained point has a fully contained open neighborhood.
