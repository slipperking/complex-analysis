...
#proof[
  ...
  $
  For the fixed value of $epsilon$, the collection ${D(zeta_k, delta)}_(k in NN)$ forms an open cover of $K$, and by the Heine--Borel Theorem (@thm:heineborel), it admits finite subcovering ${D(zeta_k, delta)}_(k in {1, dots, l})$ for some finite $l = l(epsilon) in NN$.
...
#proof[
  ...
  Let $K subset U$ be compact and arbitrary. The collection ${D(z, R_z)}_(z in K)$ forms an open cover of $K$ and by the Heine--Borel Theorem (@thm:heineborel) admits a finite subcover ${D(z_k, R_(z_k))}_(k in NN_(<= n))$ for some finite $n in NN$. If we let $delta = min_(k in NN_(<= n)) (delta_k)$, it follows that $cal(F)$ is uniformly equicontinuous on $K$. By the Arzelà--Ascoli Theorem (@thm:arzelaascoli), any sequence ${f_n}_(n in NN) subset.eq cal(F)$ has a uniformly convergent subsequence ${f_(n_k)}_(k in NN)$ on $K$.
