...
#lbl(
  ...
  <thm:identity-accumulation-of-zeros>,
)
...
#proof[
  ...
  #lbl($ f(z) = sum_(n = 0)^oo a_n (z - z_infinity)^n. $, <eq:identity-accumulation-of-zeros_taylorexpansion>)

  By @def:accumulation-point, $exists N in NN$ such that $forall n > N$, $z_n in D(z_infinity, epsilon)$. Since $z_n$ is a zero of $f$, $f (z_n) = 0$. Then, by the continuity of $f$,
  $ lim_(n -> oo) f (z_n) = f(lim_(n -> oo) z_n) = f (z_infinity) = 0. $
  Using this result in comparison to @eq:identity-accumulation-of-zeros_taylorexpansion, we get that $a_0 = 0$.
...
#proof[
  Let $h = f - g$ be holomorphic over $U$. Since $S$ has an accumulation point in $U$, and $h equiv 0$ over $S$, then by @thm:identity-accumulation-of-zeros, $h equiv 0$ over $U$.
]
...
#lbl(
  ...
  <thm:argument-principle-holomorphic>,
)
...
#proof[
  ...
  By Cauchy--Goursat (@thm:cauchy-goursat-theorem),
  ...
  The Cauchy--Goursat Formula (@thm:cauchy-goursat-formula) gives
...
#lbl(
  ...
  <thm:hurwitz-simple-case>,
)

#proof[
  By the holomorphy of $f_n (z)$, for any simple closed rectifiable curve $gamma subset U$ (whose interior is a subset of $U$), by the Cauchy--Goursat Theorem (@thm:cauchy-goursat-theorem),
  $ integral.cont_Gamma f_n (zeta) dzeta = 0. $
  Since $gamma$ is a subset of any compact subset of $U$, ${f_n (zeta)}$ uniformly converges on $gamma$, and by @thm:limit-integral-switch,
  ...
  #lbl(
    ...
    $,
    <eq:hurwitz-simple-case-integral-limit-switch-for-holomorphy>,
  )
  ...
  Therefore, $(f'(z)) / (f(z))$ is uniformly convergent on $gamma$. By @thm:limit-integral-switch, we can pass the limit through the integral in @eq:hurwitz-simple-case-argument-principle. Then,

  $ lim_(n -> oo) integral.cont_gamma (f'_n (z)) / (f_n (z)) dz = integral.cont_gamma (f'(z)) / (f(z)) dz = 0. $
  By the Argument Principle (@thm:argument-principle-holomorphic), $f(z)$ has no zeros in the interior of $gamma$. Since $gamma$ was arbitrarily chosen, either $f(z) equiv 0$ on $U$ or has no zeros in $U$.
]
...
#proof[
  ...
  Let $k_f$ and $k_g$ denote the number of zeros of $f$ and $g$ enclosed by $gamma$, respectively. By the Argument Principle (@thm:argument-principle-holomorphic),
...
We also have the following generalization of @thm:hurwitz-simple-case, which is a heuristic restatement of @thm:hurwitz-shifts:
