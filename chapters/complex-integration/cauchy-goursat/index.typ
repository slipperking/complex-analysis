...
#lbl(
  theorem[Green's Theorem, Complex Form][
    ...
    #lbl($ integral_(partial U)omega=integral_U dif omega. $, <eq:complex-green>)
  ],
  <thm:complex-green>,
)
...
#proof[
  ...
  with a singularity at $zeta=z$. Consider the region $U without D(z,epsilon)$. Since $f in C^1 (overline(U))$, by applying Green's Theorem (@thm:complex-green),
...
#proof[
  ...
  Alternatively, we can use Green's Theorem (@thm:complex-green) with $omega=f(zeta)dzeta$:
...
#figure-wrapper(
  lbl(
    ...
    <fig:cauchy-goursat_theorem_simplified_region>,
  ),
)
#proof[
  ...
  #lbl(
    ...
    $,
    <eq:cauchy-goursat-theorem_inner-inner-horizontal-to-outer-inner-horizontal-1>,
  )
  ...
  #lbl(
    ...
    $,
    <eq:cauchy-goursat-theorem-inner-inner-horizontal-to-outer-inner-horizontal-2>,
  )
  ...
  #lbl(
    ...
    $,
    <eq:cauchy-goursat-theorem-inner-inner-vertical-to-outer-inner-vertical>,
  )

  Then letting $epsilon -> 0$, for the same reason as @eq:cauchy-goursat-theorem-inner-inner-vertical-to-outer-inner-vertical, @eq:cauchy-goursat-theorem_inner-inner-horizontal-to-outer-inner-horizontal-1, @eq:cauchy-goursat-theorem-inner-inner-horizontal-to-outer-inner-horizontal-2 yield
  $
    integral_sray(M_1 N_1) f(z) dz -> integral_sray(M N) f(z) dz, integral_sray(Q_1 P_1) f(z) dz -> integral_sray(Q P) f(z) dz.
  $

  We are left to show the subsequent limits of the results from @eq:cauchy-goursat-theorem-inner-inner-vertical-to-outer-inner-vertical. For the left integral, let $y_(phi.alt) = max(phi.alt(a), phi.alt(a + epsilon))$ and $y_psi = max(psi(a), psi(a + epsilon))$.
  ...
  theorem[Cauchy--Goursat][
    ...
    #lbl(
      $
        f(z) = 1 / (taui) integral.cont_(partial U) (f(zeta)) / (zeta - z) dzeta.
      $,
      <eq:cauchy-goursat-formula>,
    )
  ],
  <thm:cauchy-goursat-formula>,
)

#proof[
  By the Cauchy--Goursat Theorem (@thm:cauchy-goursat-theorem),
  $
...
#remark[
  In the proof of @thm:pompeiu, we used Lipschitz continuity for a smooth function, which was a stronger condition than necessary. The true necessity of smoothness was to be able to apply Green's Theorem (@thm:complex-green).
]
...
#lbl(
  ...
  <ex:cauchy-goursat-formula-zeros-of-unity>,
)

#solution[to @ex:cauchy-goursat-formula-zeros-of-unity][
  ...
  #lbl(
    ...
    $,
    <eq:cauchy-goursat-formula-zeros-of-unity>,
  )

  where $c_j$ are the coefficients of the partial fraction decomposition. By the Cauchy--Goursat Formula (@thm:cauchy-goursat-formula), @eq:cauchy-goursat-formula-zeros-of-unity becomes:
...
#lbl(
  ...
  <ex:cosine-power-definite-integral>,
)

#solution[to @ex:cosine-power-definite-integral][
...
#lbl(
  proposition[Cauchy--Goursat Formula on the Exterior][
    Let $gamma subset CC$ be a simple closed curve, and suppose that $f:jexterior(gamma)-> CC$ is holomorphic and continuous on $overline(jexterior(gamma))=CC without jinterior(gamma)$, where $jinterior$ and $jexterior$ respectively denote the interior and exterior as in @thm:jordan-curve.
  ...
  <ex:cauchy-goursat-formulaexterior>,
)
...
#proof[
  + By the compactness of $gamma$, it can be completely contained within a sufficiently large disk centered at the origin ($gamma subset D(0,R)$). Then by applying @thm:cauchy-goursat-formula or @thm:cauchy-goursat-theorem on the set $D(0,R)inter jexterior(gamma)=D(0,R)without overline(jinterior(gamma))$, we get that
    ...
    $
    as expected.
  + Under the partial fraction decomposition of @eq:cauchy-goursat-formula-exterior-part-2-statement, we get that
