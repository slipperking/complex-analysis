...
#show: docs-subsubchapter.with(
  title: [Solution to the Dirichlet Problem on a Disk],
  route: "solution-to-dirichlet-problem-on-disk",
)
...
#lbl(
  theorem[
    ...
    #lbl(
      ...
      <eq:dirichlet-problem-with-laplace-equation-solution-disk>,
    )
    where $zeta = R ee^(ii tau)$.
  ],
  <thm:dirichlet-problem-with-laplace-equation-solution-disk>,
)
#figure-wrapper([
  #lbl(
    figure(
      {
        let radius = 5
        quick-plot(x-max: radius + .5, y-max: radius + .5, _canvas: {
          import cetz.draw: *
    ...
    <fig:dirichlet-problem-with-laplace-equation-solution-disk-second-integral>,
  )
])
#proof[
  Since
  ...
  $
  from @eq:laplacian-complex-form, we have that $laplacian_z P(zeta, z) = 4 pdv(P(zeta, z), z, overline(z)) = 0$ (since each term is independent of either $z$ or $overline(z)$). Moreover, by @thm:leibniz-integral-rule, @eq:dirichlet-problem-with-laplace-equation-solution-disk gives that
  $
    laplacian u(z)
  ...
  #lbl(
    ...
    <eq:dirichlet-problem-with-laplace-equation-solution-disk-limit-to-boundary>,
  )
  ...
  For a constant harmonic function identically equal to $1$, we get $integral_0^(2 uppi) P(zeta, z) dif tau = 1$ from @eq:poisson-integral-formula-2. Hence,
  $
    abs(phi(xi) - u(z))
  ...
  #lbl($ abs(theta - tau) > delta / 2 $, <eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-1>)
  and
  #lbl(
    $ (rho > R / 2) quad "and" quad eta <= R / 2 $,
    <eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-2>,
  )
  as in @fig:dirichlet-problem-with-laplace-equation-solution-disk-second-integral. Then,
  $
  ...
  #lbl(
    ...
    $,
    <eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-3>,
  )

  From @fig:dirichlet-problem-with-laplace-equation-solution-disk-second-integral, it is evident that $R - rho < abs(xi - z) < eta$. For @eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-1 to be true, we previously had that $abs(theta.alt - theta) < delta / 2$. In other words
  ...
  $
  under which @eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-1, @eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-2, and @eq:dirichlet-problem-with-laplace-equation-solution-disk-constraint-3 are satisfied.

  Hence, $forall epsilon > 0$, $exists eta > 0$ such that $forall z$ with $0 < abs(xi - z) < eta$, we have $abs(phi(xi) - u(z)) < 2 epsilon$. Then @eq:dirichlet-problem-with-laplace-equation-solution-disk-limit-to-boundary follows.
