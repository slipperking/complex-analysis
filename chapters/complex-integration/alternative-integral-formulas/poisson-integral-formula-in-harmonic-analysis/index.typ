...
This series may diverge. Observe that continuity of $u$ on the compact set $partial DD$ implies boundedness: $exists M > 0$ such that $abs(u(ee^(ii theta))) <= M$ for all $theta$ (@thm:continuous-function-bounded-on-compact). Consequently, $abs(a_n) <= M$. Introducing factors $rho^(abs(n))$ with $abs(rho) < 1$ yields a convergent series:
