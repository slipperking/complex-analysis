...
#proof[
  ...
  The calculations in @thm:schwarzahlforspick on $tilde(u)_r (z)$ (whose curvature calculations are now valid by $C^2$) give that $f^* rho_(tau_r) (tau_r)^2 - lambda_r (tau_r)^2 <= 0$, which implies $M_r <= 1$. (We have used the supporting metric, rather than $rho$, to derive this inequality) The rest of the theorem follows naturally.
]
...
Consider an entire function $f:CC -> U$, where $U$ is an unbounded region such that $CC without U$ has positive area. Fix $zeta in jinterior(CC without U)$. Then the map $z |-> 1 / (z - zeta)$ maps $U$ to a bounded region and hence $z |-> 1 / (f(z) - zeta)$ is constant by Liouville's Theorem (@thm:liouville), implying the constancy of $f$ (the essential proof of @thm:casoratiweierstrassentire).
