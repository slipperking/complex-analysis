...
Let ${a_n}_(n in NN) subset.eq extcomplex$ be a sequence. Then we say $a_n -> a_(oo)$ _spherically_ iff $forall epsilon.alt > 0$, $exists N in NN$ such that $forall n > N$, $d_sigma (a_n, a_(oo)) < epsilon.alt$, where $d_sigma$ is the spherical distance.
...
#proof[
  ...
  Hence, $forall epsilon.alt > 0$, $forall abs(z_1 - z_2) < epsilon.alt / M$,
  $ d_sigma (psi(z_1), psi(z_2)) < epsilon.alt. #qedhere $
]
...
#proof[
  ...
  where $gamma$ joins $f(z_1)$ and $f(z_2)$ where $z_1, z_2 in K$. Hence, $forall epsilon.alt > 0$, $forall z_1, z_2 in K$ such that $abs(z_1 - z_2) < epsilon.alt / M$, $d_sigma (f(z_1), f(z_2)) < epsilon.alt$, and hence $cal(F)$ is _uniformly spherically equicontinuous_. Since $d_sigma <= uppi$ for any two points by geometry of $S^2$, $cal(F)$ is also _uniformly spherically bounded_ (the compactness of $S^2$). Then the Arzelà--Ascoli Theorem (@ thm:arzelaascoli) under the spherical metric gives that $cal(F)$ is a normal family.
  ...
  1. If $f(z) != oo$, then by continuity, $exists delta > 0$ such that $forall zeta in D(z, delta)$,
     ...
     By holomorphy of $f'$ on $overline(D(z, delta / 2))$, $exists M' > 0$ such that $sup_(zeta in overline(D(0, delta / 2))) abs(f'(zeta)) < M'$. Uniform convergence of ${f'_(n_k)}_(k > N)$ gives the existence of some $N' > N$ such that $forall k > N'$,
     ...
     $ M = max(({M' + 1} union {sup_(zeta in overline(D(0, delta / 2))) abs(f'_(n_k) (zeta)) : N < k <= N'})) $
  ...
  2. $f(z) = oo$, then by continuity, $exists delta > 0$ such that $forall zeta in D(z, delta)$,
     ...
     $ M = max(({M' + 1} union {sup_(zeta in overline(D(0, delta / 2))) abs(g'_(n_k) (zeta)) : N < k <= N'})) $
     on this compact disk. Hence, $forall k > N$,
...
#proof[
  ...
  $ tilde(cal(F)) = {phi compose f}_{f in cal(F)} $
  ...
  Let $zeta in Omega$ be arbitrary and let $r_zeta > 0$ satisfy $D(zeta, r_zeta) subset.eq Omega$. By @ cor:generalizedahlfors, the pullback of $mu$ from $CC without {0, 1}$ to $D(zeta, r_zeta) subset.eq Omega$ satisfies
  $ f^* mu(z) <= lambda_(r_zeta) (z - zeta) ==> mu(f(z)) abs(f'(z)) <= (2 r_zeta)/(r_zeta^2 - abs(z - zeta)^2) quad forall z in D(zeta, r_zeta), f in tilde(cal(F)). $
  Since $forall w in CC without {0, 1}$,
  $
    (sigma)/(mu) = ((2)/(1 + abs(w)^2))/((sqrt(1 + abs(w)^((1)/(3))) sqrt(1 + abs(w - 1)^((1)/(3))))/(abs(w)^((5)/(6)) abs(w - 1)^((5)/(6)))) -> cases(
      0 & "as" w -> 0,
      0 & "as" w -> 1,
  ...
  $ f^sharp(z) = sigma compose f(z) abs(f'(z)) <= M mu compose f(z) abs(f'(z)) <= (2 r M)/(r_zeta^2 - abs(z - zeta)^2) $
  ...
  for some $n in NN$. Then ${f^sharp}_{f in tilde(cal(F))}$ is uniformly bounded on $K$ by
  $ M_K = max({(8 M)/(3 r_(zeta_k))}_{1 <= k <= n}) $
...
#proof[
  ...
  By @prop:linearfractionaltransformationuniformlysphericallycontinuous, $forall epsilon.alt > 0$, $exists delta > 0$ such that $forall abs(w_1 - w_2) < delta$ in $CC$,
  $ d_sigma (phi^(-1) (w_1), phi^(-1) (w_2)) < epsilon.alt. $
  ...
  $ d_sigma (phi^(-1) compose tilde(f)_(n_k) (z), phi^(-1) compose tilde(f)(z)) = d_sigma (f_(n_k) (z), f(z)) < epsilon.alt. $
