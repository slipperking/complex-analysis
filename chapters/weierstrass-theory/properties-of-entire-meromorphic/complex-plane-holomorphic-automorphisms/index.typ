...
#show: docs-subsubchapter.with(
  ...
  route: "complex-plane-holomorphic-automorphisms",
)
...
#proof[
  ...
  We will now prove the primary assumption; the singularity at $z = oo$ cannot be an essential singularity of $f(z)$. Let $w in CC$ be arbitrary. Then by the Casorati--Weierstrass Theorem (@thm:casorati-weierstrass), $forall epsilon > 0$ and $forall R > 0$, $exists abs(z) > R$ such that $abs(f(z) - w) < epsilon$. Equivalently, $forall R > 0$, $exists zeta in D(w, epsilon)$ such that $abs(f^(-1) (zeta)) > R$. Since $f^(-1)$ is continuous on $overline(D(w, epsilon))$ by holomorphy, by @thm:continuous-function-bounded-on-compact, it is bounded, which is a contradiction.
]
