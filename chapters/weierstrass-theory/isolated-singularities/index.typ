...
#show: docs-subchapter.with(
  ...
  route: "isolated-singularities",
  children: [
    #include "at-infinity/index.typ"
...
+ If $lim_(z -> a) f(z)$ exists and is finite, then $z = a$ is a removable singularity and can be analytically continued to $D(a, r)$ by @thm:riemann-removable-singularities. Consequently, $f(z)$ has a convergent Taylor expansion and the principal part of its Laurent expansion vanishes, and $f(z) = phi(z)$. #enum-lbl(<itm:isolated-singularities-removable>)
+ If $lim_(z -> a) f(z) = oo$, then $z = a$ is a _pole_ of $f$ (from the stereographic projection and the Riemann sphere, the $oo$ is a single point in $extcomplex$, and approaching $oo$ does not distinguish between different directions, unlike the use of $+oo$ and $-oo$). #enum-lbl(<itm:isolated-singularities-pole>)

  #lbl(
    theorem[
      The condition $lim_(z -> a) f(z) = oo$ is equivalent to there being a finite number of nonzero $c_(-n)$'s, where $n in NN$.
    ],
    <thm:isolated-singularities-pole-laurent-expansion>,
  )
  ...
  #proof[
    ...
    $ g(z) = 1 / f(z). $
    Then $lim_(z -> a) g(z) = 0$. There exists a $delta > 0$ such that $f$ is nonzero on $D^*(a, delta)$. Then $g(z)$ is holomorphic on $D^*(a, delta)$ and has a removable singularity at $z = a$. By @thm:riemann-removable-singularities, $g$ can be analytically continued to $D(a, delta)$. Let the multiplicity of the zero at $z = a$ be $m$. Then
    $ g(z) = phi(z) (z - a)^m, $
...
+ If $lim_(z -> a) f(z)$ does not exist, then $a$ is known as an _essential singularity_. #enum-lbl(<itm:isolated-singularities-essential>)

  #lbl(
    example[
      Classify the singularity of the function $ee^(1 / z)$ on the punctured plane $CC^*=CC without {0}$ at $z = 0$.
    ],
    <ex:isolated-singularities-essential-exp-1-z>,
  )

  #solution[to @ex:isolated-singularities-essential-exp-1-z][
  ...
  This follows by elimination from the established trichotomy; if the limit as $z -> a$ does not exist, then the singularity is neither removable nor a pole (results from @itm:isolated-singularities-removable and @itm:isolated-singularities-pole). Similar logic can be applied to the coefficients of the Laurent expansion.

  Indeed, in @ex:isolated-singularities-essential-exp-1-z, the Laurent expansion is equal to:
...
#lbl(
  theorem([Casorati--Sokhotski--Weierstrass], restate: true, restate-keys: ("thm:casorati-weierstrass",))[
...
#proof[
  ...
  $ g(z) = (f(z) - w) / (z - a), $
  which is holomorphic and non-vanishing on the punctured neighborhood of $a$. Since as $z -> a$, $g(z) -> oo$, it follows that $g(z)$ has a pole at $a$. Let the order of the pole be $m in NN$. By @thm:isolated-singularities-pole-laurent-expansion, $g(z)$ has the Laurent expansion
...
#lbl(
  ...
  <thm:casorati-weierstrass-entire>,
)
...
In @sec:differential-geometry, we will prove a profound generalization of the two results (@thm:great-picard and @thm:little-picard), which was first proved by Emile Picard in 1879:
#thm-state.thm-restate("thm:little-picard", final: true)
#thm-state.thm-restate("thm:great-picard", final: true)
