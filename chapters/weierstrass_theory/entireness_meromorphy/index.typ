...
We have previously defined the concept of an entire function in the chapter on complex differentiation. Let $f$ be entire with the unique Taylor expansion $sum_(n = 0)^infinity c_n z^n$. Since $z = infinity$ is an isolated singularity, by the uniqueness of the Laurent expansion, the expansion at $z = 0$ has the same form as the expansion at $z = infinity$. We will now analyze the implications on the entire function $f$ given an isolated singularity.

1. If the infinity point is a removable singularity, then $lim_(z -> infinity) f(z)$ exists and is finite.

  #proposition[
    If $f(z)$ is entire and has a removable singularity at $z = infinity$, then $f$ is constant.
...
2. If $f(z)$ has a pole at $z = infinity$ of order $m in NN$, then $f$ is a polynomial of degree $m$.

  #proof[
    By the classification of a pole at $infinity$, $f$ can be written as
    $
...
3. If $f(z)$ has an essential singularity at $z = infinity$, $f(z)$ is known as a _transcendental entire function_.
...
#theorem[
  If $f(z)$ is meromorphic on $CC$ and has a pole or removable singularity at $z = infinity$, then $f$ is a rational function.
...
#proof[
  ...
  $
    f(z) = sum_(n = -infinity)^infinity a_n z^n,
  $
  where the series converges for sufficiently large $abs(z)$. If $infinity$ is a removable singularity, the coefficients $a_n = 0$ for all $n > 0$. If $infinity$ is a pole of order $m$, then $a_n = 0$ for all $n > m$, and $a_m != 0$. In either case, the principal part at $infinity$ is
  $
  ...
  Since $f(z) - psi_infinity(z)$ is the holomorphic part of the expansion at $infinity$, consisting of terms with nonpositive powers of $z$, $lim_(z -> infinity) f(z) - psi_infinity(z)$ exists and is finite. Additionally, each $psi_k(z)$ consists of negative powers of $z - z_k$, so $lim_(z -> infinity) psi_k(z) = 0$ for each $k$, and thus $lim_(z -> infinity) sum_(k = 1)^n psi_k(z) = 0$. Therefore, $lim_(z -> infinity) Phi(z)$ exists and is finite, so $infinity$ is a removable singularity of $Phi$. Without the finite singularities at each $z_k$, $Phi$ is entire. Since $Phi$ has a finite limit at $infinity$, it is bounded on $CC$. By Liouville's theorem, $Phi(z) equiv c$ for some constant $c$.

  Hence,
  $
    f(z) = c + psi_infinity(z) + sum_(k = 1)^n psi_k(z).
  $
...
If $z = infinity$ is not a pole or removable singularity of a meromorphic function $f(z)$, then it is either an essential singularity or an accumulation point of poles. In this case, $f$ is not rational and is known as a _transcendental meromorphic function_.
