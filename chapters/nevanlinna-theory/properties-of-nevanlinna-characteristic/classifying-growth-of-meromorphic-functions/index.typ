...
#show: docs-subsubchapter.with(
  ...
  route: "classifying-growth-of-meromorphic-functions",
)
#lbl(
  theorem[
  ...
  <thm:nevanlinna-entire-function-maximum-modulus-sandwich>,
)
...
#proof[
  ...
  $
  which proves the first inequality. Since $T >= 0$ trivially, if $M <= 1$ the second assertion holds trivially. Hence, assume $M > 1$; by the Poisson--Jensen formula (@thm:poisson-jensen-formula), letting $z in partial D(0, r)$ be where $abs(f)$ attains $M$, we have
  $
...
#lbl(
  ...
  <def:order-of-meromorphic-function>,
)
...
#proof[of equivalent definitions][
  ...
  $
  which is a contradiction. The converse follows similarly to the case of entire functions in @sec:classifying-growth-of-entire-functions.
]

#theorem[
  Let $rho_M$ be the order of a non-constant entire function $f$ as defined in @sec:classifying-growth-of-entire-functions and let $rho_T$ be the order as defined in @def:order-of-meromorphic-function. Then $rho_M = rho_T$.
]

#proof[
  Since $f$ is non-constant, for sufficiently large $r$, $logp M(r, f) = log M(r, f)$. For the remainder of this proof we will assume the two are equivalent. From @thm:nevanlinna-entire-function-maximum-modulus-sandwich it is apparent that (under $R = 2r$ for sufficiently large $r$)
  $
...
#lbl(
  proposition[
    ...
    + $m(r, a, f) = Order(r^(rho + epsilon))$. #enum-lbl(<itm:meromorphic-function-finite-order-estimates-proximity>)
    + $N(r, a, f) = Order(r^(rho + epsilon))$. #enum-lbl(<itm:meromorphic-function-finite-order-estimates-counting>)
    + $n(r, a, f) = Order(r^(rho + epsilon))$. #enum-lbl(<itm:meromorphic-function-finite-order-estimates-discrete-counting>)
  ],
  <prop:meromorphic-function-finite-order-estimates>,
)

#proof[
  By the First Fundamental Theorem of Nevanlinna theory (@thm:nevanlinna-first-fundamental-theorem),
  ...
  $ T(r, f) = Order(r^(rho + epsilon)), $
  which proves @itm:meromorphic-function-finite-order-estimates-proximity and @itm:meromorphic-function-finite-order-estimates-counting. Since
  $
    N(2r, a, f) - N(r, a, f) & = integral_r^(2r) (n(x, a, f)) / x dx \
                             & >= n(r, a, f) integral_r^(2r) dx / x = log 2 dot n(r, a, f),
  $
  it follows from @itm:meromorphic-function-finite-order-estimates-counting that
  $
    n(r, a, f) <= (N(2r, a, f) - N(r, a, f)) / (log 2) <= N(2r, a, f) / (log 2) = Order((2r)^(rho + epsilon)) = Order(r^(rho + epsilon))
  $
  and hence @itm:meromorphic-function-finite-order-estimates-discrete-counting follows.
]
...
#lbl(
  ...
  <thm:meromorphic-function-finite-order-estimates-sum>,
)
...
#proof[
  ...
  and hence the conclusion follows for $a = oo$ by @thm:nonzero-sequence-power-summation-convergence. Assume $a$ is finite; observe that
  $ T(r, f) = T(r, a, f) + Order(1) = T(r, frac(1, f - a)) + Order(1) $
  by the First Fundamental Theorem (@thm:nevanlinna-first-fundamental-theorem). It follows that $rho$ is the order of $1/(f-a)$. Applying the previous result to this function, whose poles are precisely at ${ r_n (a) }$, the conclusion follows.
]
