...
#theorem[
  Let $f : CC -> extcomplex$ be non-constant and meromorphic of finite order $rho$ such that $f(0) != 0, oo$. If ${a_n}_(n=1)^oo$ and ${b_n}_(n=1)^oo$ are its zeros and poles in $CC^*$ listed in order of non-decreasing moduli, counting multiplicities and zeros, then the products
  $
...
#proof[
  By @thm:meromorphic-function-finite-order-estimates-sum, letting $q = floor(rho)$ gives that the two given products converge locally uniformly and the corresponding summation of exponent $q + 1$ converge. Define $g$ to be the analytic continuation across all removable singularities of $f dot P_2$ such that $g$ is entire and vanishes at each ${a_n}$. Observe that
  $ T(r, g) <= T(r, f) + T(r, P_2), quad T(r, f) = Order(r^(rho + epsilon)) $
  by @prop:nevanlinna-characteristic-properties and @prop:meromorphic-function-finite-order-estimates. For sufficiently large $r$,
  ...
  $
  where $c$ is a constant dependent only on $q$ and $r = abs(z)$ (see @eq:infinite-product-weierstrass-factorization-uniform-bound). Since for sufficiently small $epsilon > 0$, $q + 1 > rho + epsilon$,
  $
  ...
  $
  (the summation of converges by @thm:meromorphic-function-finite-order-estimates-sum). Assume now that $abs(b_n) <= 2r$, or $2 abs(z \/ b_n) >= 1$. Then,
  $
  ...
  and thus $g$ is of order $rho' <= rho$. For any $epsilon$, there is some $c$ such that $log(1 + abs(w)) <= c abs(w)^epsilon$; thus for $q = 0$, the assertion is direct from @prop:meromorphic-function-finite-order-estimates:
  $
  ...
  $
  Similar logic then derives that $g$ is of order $<= rho$. There then exists an integer $p <= rho'$ such that $P_1$ converges and $g(z) = ee^(phi(z)) P_1 (z)$ has the property that $phi$ is a polynomial of degree $g <= rho'$ by the Hadamard Factorization Theorem (@thm:hadamard-factorization) for entire functions. Writing $f = frac(g, P_2, style: "horizontal")$ then yields the desired representation.
]
