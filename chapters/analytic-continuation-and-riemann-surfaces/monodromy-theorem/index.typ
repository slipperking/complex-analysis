...
#proof[
  ...
  $
  Let us fix $s in S$. The analytic continuation ${(f_(s, t), U_(s, t))}_(0 <= t <= 1)$ along the curve $t |-> H(s, t)$ generates the cover ${U_(s, t)}_(0 <= t <= 1)$ of the compact curve ${H(s, t)}_(0 <= t <= 1)$. By Heine--Borel (@ thm:heineborel), there exists a finite subcover ${U_(s, t_k)}_(k=1)^n$. Then $exists epsilon > 0$ such that each $D(H(s, t), epsilon) subset.eq union.big_(k=1)^n U_(s, t_k)$ for all $t in [0, 1]$. In fact, we can choose $epsilon$ to be
  $ epsilon = min_(k=1)^n [op("dist")(H(s, [0, 1]), diff U_(s, t_k) without union.big_(j=1)^n U_(s, t_j))]. $
  It can be verified that $op("dist")$ here is positive since $H(s, [0, 1]) subset union.big_(j=1)^n U_(s, t_j)$ (thus both sets are disjoint) and both are compact sets. By continuity, $forall t in [0, 1]$, $exists delta > 0$ such that $forall s' in [0, 1] inter (s - delta, s + delta)$, $abs(H(s, t) - h(s', t)) < epsilon$. By the Heine--Cantor Theorem (@ thm:heinecantor), $delta$ attains a positive infimum, and thus $exists delta > 0$ such that $forall s' in [0, 1] inter (s - delta, s + delta)$, $forall t in [0, 1]$, $abs(H(s, t) - H(s', t)) < epsilon$.

  Fix $s'$ and $t$. By the derived inequality, we have that $H(s', t) in U_(s, t)$ and therefore, $tilde(U)_(s', t) = D(H(s', t), epsilon - abs(H(s, t) - H(s', t))) subset.eq U_(s, t)$. Thus, the analytic function element $(f_(s, t), tilde(U)_(s', t))$ is a direct analytic continuation of $(f_(s, t), U_(s, t))$. By constructing analytic function elements similarly for all $t$, we obtain an analytic continuation of $(f_(s, 0), tilde(U)_(s', 0))$ along $H(s', [0, 1])$. Because $(f_(s, 0), U_(s, 0))$ and $(f_(s, 0), tilde(U)_(s', 0))$ are direct analytic continuations of each other, by @ lem:analyticcontinuationalongcurveuniqueness, the continuations ${(f_(s, t), tilde(U)_(s', t))}_(0 <= t <= 1)$ and ${(f_(s', t), U_(s', t))}_(0 <= t <= 1)$ are equivalent. Thus, all elements of $(s - delta, s + delta) inter [0, 1]$ belong in $S$, and thus $S$ is relatively open in $[0, 1]$.

  Let ${s_n}_(n in NN) subset S$ be an arbitrarily chosen convergent sequence accumulating at $s_oo$. Since there exists an analytic continuation of $(f_(s_oo, 0), U_(s_oo, 0))$ along the curve $H(s_oo, [0, 1])$, we can use the same argument as before to construct $epsilon$ such that each $D(H(s_oo, epsilon)) subset.eq U_(s_oo, t)$ and $delta$ such that $forall s in [0, 1] inter (s_oo - delta, s_oo + delta)$, $forall t in [0, 1]$, $abs(H(s, t) - H(s_oo, t)) < epsilon$. By convergence, for sufficiently large $n$, $abs(s_n - s_oo) < delta$. Hence, there is an analytic continuation
  $ {(f_(s_oo, t), D(H(s_n, t), epsilon - abs(H(s_n, t) - H(s_oo, t))))}_(0 <= t <= 1) $
  along $H(s_n, [0, 1])$, which is equivalent to ${(f_(s_n, t), U_(s_n, t))}_(0 <= t <= 1)$ by @ lem:analyticcontinuationalongcurveuniqueness under the same preliminary assumptions as shown previously. By the following logic, $f_(s_oo, 1) equiv f_(s_n, 1)$ on the intersection, and $s_oo in S$ and $S$ is closed. Hence, by @ thm:connectedtopologicalspaceclopensets, $S = [0, 1]$. Thus, $forall 0 <= lambda <= 1$, $f_(lambda, 1) equiv f_(0, 1)$ on $U_(lambda, 1) inter U_(0, 1) in.rev Q$.
]
