...
#proof[
  ...
  and $z_infinity in S$. Thus, $S$ contains all of its accumulation points in $V$ and is therefore closed (if $z_infinity not in V$, then it is no longer relevant; we are concerned with its relative closedness in $V$).
