...
#proof("Sketch of proof")[Let $K_0$ and $K_1$ be the two components of $hat(CC)without U$, where $hat(CC)=CCunion{infinity}$ is the Riemann sphere. By a preliminary Möbius transformation we may assume $0 in K_0$ and $infinity in K_1$.
    ...
    _Step 2: Uniformisation._ The projection $tilde(U)-> U tilde.equiv tilde(U)/angle.l T angle.r$ then realises $U$ as the quotient of a half-plane by $z tilde.op lambda z$. The map $z |-> ee^{taui (log z)/(log lambda)}$ descends to a biholomorphism from $U$ to the annulus $A (1,lambda)$.
