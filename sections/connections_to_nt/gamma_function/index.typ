...
$ Gamma(z)=integral_0^infinity ee^{-t}t^{z-1}dif t, $ <eq:gammafunction>
 where $z in CC$.] <def:gammafunction>

By letting $z=x+ii y$ where $x,y in RR$, we have $|ee^{-t}t^{z-1}|=ee^{-t}t^{x-1}$. Notice that for $x>0$,

$
|Gamma(x)| & =integral_0^1 ee^{-t}t^{x-1}ddt+integral_1^infinity ee^{-t}t^{x-1}ddt \\ & lt.eq integral_0^1 t^{x-1}ddt+integral_1^infinity ee^{-t}t^{x-1}ddt=(1)/(x)+integral_1^infinity ee^{-t}t^{x-1}ddt.
$

Since $integral_1^infinity (ddt)/(t^2)$ is convergent and $\lim_{t -> infinity}(ee^{-t}t^{z-1})/(t^{-2})=0$, then by comparison, the second integral is convergent.
...
$
oint.cont_gamma Gamma(z)ddz & =oint.cont_gamma (integral_0^infinity ee^{-t}t^{z-1}ddt)ddz=integral_0^infinity ee^{-t}(oint.cont_gamma t^{z-1}ddz)ddt=0,
$
...
$ Gamma(z+1)=integral_0^infinity ee^{-t}t^z ddt=-\eval{ee^{-t}t^z}_0^infinity+z integral_0^infinity ee^{-t}t^{z-1}ddt.=z Gamma(z). $

Additionally, 
$ Gamma(1)=integral_0^infinity ee^{-t}ddt=-\eval{ee^{-t}}_0^infinity=1. $
...
#theorem("#smallcaps[Gauss]")[The Gamma function satisfies
    
$ Gamma(z)=\lim_{n -> infinity}(n^z n!)/(product_{k=0}^n(z+k)), Re z>0. $ <eq:gammafunctiongaussformula>] <thm:gammafunctiongaussformula>
...
$ \lim_{n -> infinity}[integral_0^n ee^{-t}t^{z-1}ddt-f_n(z)]=\lim_{n -> infinity}integral_0^n ee^{-t}t^{z-1}[1-ee^{t}{(1-(t)/(n))}^n]ddt. $ <eq:gammafunction_gaussformulaintermediate1>
...
$ 1-ee^t (1-(t)/(n))^n=(1)/(n)integral_0^t u ee^u (1-(u)/(n))^{n-1}dif u. $ <eq:gammafunction_gaussformulaintermediate2>
...
$ |integral_0^n ee^{-t}t^{z-1}[1-ee^{t}{(1-(t)/(n))}^n]ddt|lt.eq (ee)/(2n)|integral_0^n ee^{-t}t^{z+1}ddt|<(1)/(2n)|Gamma(z+2)|-> 0 $
 as $n -> infinity$. From @eq:gammafunctiongaussformulaprelimit, we have $Gamma(z)=\lim_{n -> infinity}(n^z n!)/(product_{k=0}^n (z+k))$, or @eq:gammafunctiongaussformula]
...
$ (1)/(Gamma(z))=z product_{k=1}^n [(1+(z)/(k))ee^{-(z)/(k)}]ee^{z gamma}, $ <eq:gammafunction_weierstrassformula>
 where $\upgamma=integral_1^infinity ((1)/(\floor{x})-(1)/(x))ddx$.] <thm:gammafunction_weierstrassformula>
...
$
(1)/(Gamma(z)) & =\lim_{n -> infinity}(product_{k=0}^n(z+k))/(n^z n!) \\ & =z product_{k=1}^n [(1+(z)/(k))ee^{-(z)/(k)}]\lim_{n -> infinity}exp [z \qty(integral_1^n ((1)/(\floor{x})-(1)/(x))ddx)] \\ & =z product_{k=1}^n [(1+(z)/(k))ee^{-(z)/(k)}]exp (z \upgamma).
$

    The constant $\upgamma=integral_1^infinity ((1)/(\floor{x})-(1)/(x))ddx$ is known as the _Euler--Mascheroni constant_. For each $x$, $0 lt.eq \frac1{\floor{x}}-\frac1x<\frac1{\floor{x}}-\frac1{\floor{x}+1}$, and the integral is bounded by $sum_{n=1}^infinity (\frac1n-\frac1{n+1})<1$, and hence the integral is convergent. The properties of $\upgamma$ still largely remain unknown.

    By the Weierstrass Factorization Theorem (@thm:weierstrassfactorization), if we let $a_n=-n$ and $p_n=1$, it follows that 
$ sum_{n=1}^infinity |(R)/(a_n)|^{p_n+1}=R^2 sum_{n=1}^infinity (1)/(n^2)=(R^2 pi^2)/(6) $
...
#theorem("Euler's Reflection Formula")[The analytic continuation of the $Gamma$-function can be analytically continued to the left half-plane with the functional equation of
...
$ Gamma(z)Gamma (z+(1)/(2))=2^{1-2z}sqrt(pi)Gamma(2z). $ <eq:gammafunction_legendreduplication>] <thm:gammafunction_legendreduplication>

#proof[From @thm:gammafunctiongaussformula, we have 
$ Gamma(z)Gamma (z+(1)/(2))=\lim_{n -> infinity}(n^{2z+(1)/(2)}n!^2)/(product_{k=0}^n(z+k)(z+k+(1)/(2)))=\lim_{n -> infinity}(2^{2n+2}n^{2z+(1)/(2)}n!^2)/(product_{k=0}^{2n+1}(2z+k)) $

    where the left-hand side is defined since $z in CCwithout (-(NN)/(2))$. By expansion of the value, we have
    
$
Gamma(z)Gamma (z+(1)/(2)) & =\lim_{n -> infinity}((2n)^{2z}(2n)!)/(product_{k=0}^{2n}(2z+k))dot.c (n^{(1)/(2)}n!^2 2^{2n+2-2z})/((2z+2n+1)(2n)!) \\ & =Gamma(2z)\lim_{n -> infinity}(n^{(1)/(2)}n!^2 2^{2-2z})/((2z+2n+1)product_{k=0}^{n-1}(k+(1)/(2))product_{k=1}^n k) \\ & =2^{2-2z}Gamma(2z)\lim_{n -> infinity}(n^{(1)/(2)}n!)/(product_{k=0}^n (k+(1)/(2)))dot.c (n+\frac12)/(2z+2n+1) \\ & =2^{1-2z}Gamma(2z)Gamma ((1)/(2)) \\ & =2^{1-2z}Gamma(2z)sqrt(pi),
$
...
$ Gamma(z)Gamma (z+(1)/(m))dots.c Gamma (z+(m-1)/(m))=(2 pi)^{(m-1)/(2)}m^{(1)/(2)-mz}Gamma(mz). $ <eq:gammafunction_gaussmultiplication>]
...
$
I&=integral_0^{infinity}arctan{u}(sqrt(u)dif u)/(1+u^2)=integral_0^infinity (integral_0^1 (u)/(1+u^2v^2)dif v)(sqrt(u))/(1+u^2)dif u\\ &=integral_0^1 integral_0^infinity (u^{\frac32})/((1+u^2v^2)(1+u^2))dif udif v=integral_0^1 integral_0^infinity (s^{\frac14})/(2 (1+sv^2)(1+s))dif sdif v.
$

    The integral switch is justified since the integrand $gt.eq 0$ throughout. Consider the integral for 
$ integral_0^infinity (s^{\frac14}dif s)/((1+sv^2)(s+1))=\frac1{1-v^2}integral_0^{infinity}s^{beta}(\frac1{1+s}-(1)/(c+s))dif s, c=v^{-2}, beta=\frac14 $
...
$ J(beta)=integral_0^infinity (s^beta dif s)/(1+s)-integral_0^infinity (s^beta c^{beta}dif s)/(1+s)=(1-c^beta)op("B")(-beta,beta+1)=(c^beta-1)pi csc (pi beta) $
