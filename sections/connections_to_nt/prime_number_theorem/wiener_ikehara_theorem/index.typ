...
$ K_lambda(x)= cases(1-(|x|)/(2 lambda) & |x|<2 lambda, ,
 0 & "otherwise",) op("supp") K_lambda=[-2 lambda,2 lambda], $
 and a normalized Fejér kernel by

$ k_lambda(x)= cases((2 lambda)/(sqrt(2 pi))\qty((sin(lambda x))/(lambda x))^2 & x eq.not 0, ,
...
$ cal(F)\cbraces{K_lambda}(x)=cal(F)^{-1}\cbraces{K_lambda}(x)=k_lambda(x), cal(F)\cbraces{k_lambda}(x)=cal(F)^{-1}\cbraces{k_lambda}(x)=K_lambda(x), $
 where we define 
$ cal(F)\cbraces{f}(x)=\frac1{sqrt(2 pi)}integral_{-infinity}^infinity f(t)ee^{-ii xt}ddt, cal(F)^{-1}\cbraces{f}(x)=\frac1{sqrt(2 pi)}integral_{-infinity}^infinity f(t)ee^{ii xt}ddt. $] <lem:wienerikehara_kernels_fouriertransform>
...
$
cal(F)\cbraces{k_lambda}(x) & =(2 lambda)/(2 pi)integral_{-infinity}^infinity \qty((sin(lambda t))/(lambda t))^2 ee^{-ii xt}ddt=\frac2{pi lambda}integral_{0}^infinity (sin^2(lambda t))/(t^2)cos(xt)ddt \\ & =(1)/(pi lambda)integral_0^infinity (1-cos(2 lambda t))/(t^2)cos(xt)ddt.
$

    Let $I(x)=integral_0^infinity (1-cos(2 lambda t))/(t^2)cos(xt)ddt$. Differentiation under the integral sign yields 
$ dv(I, x)=integral_0^infinity \pdv{x}\qty((1-cos(2 lambda t))/(t^2)cos(xt))ddt=integral_0^infinity \qty((cos(2 lambda t)-1)/(t))sin(xt)ddt. $
 // TODO: prove the conditions required for the commute
    By the product-to-sum formulas, this can be rewritten as 
$ dv(I, x)=integral_0^infinity (sin(xt+2 lambda t))/(2t)ddt+integral_0^infinity (sin(xt-2 lambda t))/(2t)ddt-integral_0^infinity \frac{sin(xt)}t ddt=upright("I")+\mathrm{I\!I}-\mathrm{I\!I\!I}. $

    By substituting $u=t(x+2 lambda)$, we have that 
$ upright("I")=integral_0^{plus.minus infinity}(sin u)/(2u)dif u,\qfor plus.minus infinity= cases(+infinity & x+2 lambda>0, ,
 -infinity & x+2 lambda<0,) $
 and substituting $u=t(x-2 lambda)$ gives 
$ \mathrm{I\!I}=integral_0^{plus.minus infinity}(sin u)/(2u)dif u,\qfor plus.minus infinity= cases(+infinity & x-2 lambda>0, ,
 -infinity & x-2 lambda<0.) $
 By the Dirichlet integral $integral_0^infinity \frac{sin u}u dif u=\frac pi2$, we have that 
$ upright("I")=\frac pi4 op("sgn")(x+2 lambda), \mathrm{I\!I}=\frac pi4 op("sgn")(x-2 lambda). $

    Similarly, $\mathrm{I\!I\!I}=\frac pi2 op("sgn")(x)$. Hence, 
$ dv(I, x)=\frac pi4 [op("sgn")(x+2 lambda)+op("sgn")(x-2 lambda)-2 op("sgn")(x)]=\frac{pi}4 cases(0 & |x|>2 lambda, ,
 1 & -2 lambda<x<0, ,
 -1 & 0<x<2 lambda,) $
 and 
$ I(0)+integral_0^x dv(I, t)ddt=I(0)+ cases((\mathmakebox[\widthof{\(integral^-\)}][l]{integral_0^{-2 lambda}}+integral_{-2 lambda}^x)dv(I, t)ddt & x<-2 lambda, ,
...
$ I(x)=I(0)+pi cases(-lambda & x<-2 lambda, ,
...
$
I(0) & =integral_0^infinity (1-cos(2 lambda t))/(t^2)ddt=\eval{(cos(2 lambda t)-1)/(t)}_0^infinity+integral_0^infinity 2 lambda (sin(2 lambda t))/(t)ddt \\ & =\lim_{t -> 0}(1-cos(2 lambda t))/(t)+2 lambda \frac pi2=pi lambda,
$

    we obtain 
$ I= cases(0 & x<-2 lambda, ,
 pi (lambda+\frac x2) & -2 lambda<x<0, ,
 pi (lambda-\frac x2) & 0<x<2 lambda, ,
 0 & x>2 lambda,)=> cal(F)\cbraces{k_lambda}(x)= cases(0 & x<-2 lambda, ,
...
$
(1)/(sqrt(2 pi))integral_{-infinity}^infinity k_{lambda}(t)ddt & =(lambda)/(pi)integral_{RR}\qty((sin(lambda t))/(lambda t))^2 ddt=(1)/(pi)integral_{RR}(\frac{sin t}t)^2 ddt \\ & =-\eval{(sin^2 t)/(pi t)}_{-infinity}^infinity+integral_{RR}(\dd(sin^2 t))/(pi t)=integral_{RR}(sin(2t))/(pi t)ddt=1,
$
...
#theorem("#smallcaps[Wiener--Ikehara]")[Let $f:RR_{gt.eq0}-> RR$ be a nonnegative, monotically nondecreasing function such that the Laplace transform
    
$ cal(L)\cbraces{f}(s)=integral_0^infinity f(x)ee^{-sx}ddx $ <eq:wienerikehara_laplacetransform>
...
$ \lim_{x ->+infinity}(f(x))/(ee^x)=c. $ <eq:wienerikehara_conclusion>] <thm:wienerikehara>

#proof[Let 
$ a(t)= cases(f(t)ee^{-t} & t gt.eq 0, ,
...
$ (a*k_lambda)(x)<c+1 => (1)/(pi)integral_{-infinity}^infinity ((sin t)/(t))^2a (x-\frac t lambda)ddt<c+1 $
...
#theorem("name=_Prime Number Theorem_,store=thm:primenumber")[The prime counting function $pi(x)$ is asymptotically equal to $\frac x{log x}$, i.e., $\lim_{x ->+infinity}\frac{pi(x)log x}x=1$.] <thm:primenumber>

#proof[By @thm:chebyshevfunctions_limsup_inflim_equivalences, it suffices to show that $\lim_{x ->+infinity}\frac{psi(x)}x=1$. Consider the Laplace transform of $psi compose exp$ in @eq:primenumbertheorem_laplacetransformchebyshevfunction_statement. By @thm:primenumbertheorem_laplacetransformchebyshevfunction, the function $g$ defined therein converges uniformly on compact subsets of $RR$ and is continuously differentiable. Hence, by the Wiener--Ikehara theorem (@thm:wienerikehara), we have that 
$ \lim_{x ->+infinity}(psi (ee^x))/(ee^x)=\lim_{x ->+infinity}(psi (x))/(x)=1, $
